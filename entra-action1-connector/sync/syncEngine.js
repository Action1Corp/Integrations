...
const tenantKey = includeName ? `${baseKey}:${tenantNameSafe}` : baseKey;
      ...
      const ambiguousEndpointReuseCount = mappingResult?.ambiguousEndpointReuse?.length || 0;
      ...
      logger.info(
        `[sync] ${jobLabel} matched=${mappingResult?.matched?.length || 0}, ` +
          ...
          `ambiguousEndpointReuse=${ambiguousEndpointReuseCount}, ` +
          `patchesPlanned=${planned}, patchesToProcess=${patchesToProcess.length}` +
          (limitedBy ? ` (LIMITED by ${limitedBy})` : '')
      );

      if (ambiguousEndpointReuseCount > 0) {
        const LIMIT_ENDPOINTS = 5;
        const LIMIT_CANDIDATES = 5;
        const examples = (mappingResult.ambiguousEndpointReuse || []).slice(0, LIMIT_ENDPOINTS).map((x) => {
          const shown = (x?.candidates || [])
            .slice(0, LIMIT_CANDIDATES)
            .map(
              (c) =>
                `name="${c?.entraName || ''}" id=${c?.entraId || 'n/a'} matchType=${c?.matchType || 'n/a'}`
            )
            .join(', ');
          const more = (x?.candidates || []).length > LIMIT_CANDIDATES
            ? ` ...(+${(x.candidates || []).length - LIMIT_CANDIDATES} more)`
            : '';
          return `endpointId=${x?.endpointId || 'n/a'} endpointName="${x?.endpointName || ''}" candidates=${
            x?.candidateCount || 0
          } => ${shown}${more}`;
        });

        logger.warn(
          `[sync] ${jobLabel} Endpoint reuse collisions skipped=${ambiguousEndpointReuseCount}. ${examples.join(' | ')}`
        );
      }
